...
function leerPersonasDtv(workbook: any): PersonaExcel[] {
  ...
  if (!validacion.valido) {
    throw new Error(
      `Columnas requeridas faltantes: ${validacion.faltantes.join(", ")}`
    );
  }
...
function deduplicarPersonas(personas: PersonaExcel[]): PersonaDeduplicated[] {
  ...
  for (const persona of personas) {
    ...
    const telefonoNormalizado =
      persona.telefonoPrincipalNormalizado || persona.telefonoPrincipal;
  ...
  try {
    ...
    for (const persona of personasDeduplicated) {
      ...
      if (
        ...
        persona.telefonoPrincipalNormalizado.startsWith("+54")
      ) {
    ...
    if (personasFueraRango.length > 0) {
      exportFileName = await generarExportFueraRango(
        supabase,
        campana_id,
        personasFueraRango
      );
