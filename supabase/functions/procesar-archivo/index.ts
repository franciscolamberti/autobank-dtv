...
serve(async (req) => {
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const telefonos = [row[40], row[41], row[38], row[39]].filter(t => t && String(t).trim())
      ...
      for (const punto of puntosPickit) {
        const distancia = calcularDistanciaHaversine(lat, lon, punto.lat, punto.lon)
      ...
      const persona = {
        ...
        direccion_completa: `${String(row[30] || '').trim()} ${String(row[31] || '').trim()}`.trim(),
    ...
    const dentroRango = personasParaInsertar.filter(p => p.dentro_rango).length
    await supabase
      .from('campanas')
      .update({
