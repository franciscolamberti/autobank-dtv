...
function obtenerTelefonoPrincipal(row: any): {
  telefono: string | null;
  probableFijo: boolean;
} {
...
function normalizarTelefonoE164(original: string | null): string | null {
  ...
  for (const len of [4, 3, 2]) {
    if (s.length >= len + 6) {
      // ensure reasonable local length remains
      area = s.slice(0, len);
      local = s.slice(len);
      break;
    }
...
function validarColumnasRequeridas(data: any[]): {
  valido: boolean;
  faltantes: string[];
} {
  ...
  const ensureIndex = (idx: number, name: string) => {
    if (header.length <= idx) {
      faltantes.push(`${name} (column ${idx})`);
      return;
    }

    const v = first[idx];
    if (v == null || String(v).trim() === "") {
      faltantes.push(`${name} (columna ${idx})`);
    }
  };

  ensureIndex(0, "NroCliente");
  ensureIndex(1, "NroWO");
  ensureIndex(28, "ApellidoNombre");
  ensureIndex(32, "X- Longitud");
  ensureIndex(33, "Y - Latitud");

  // Teléfono: al menos una de 38, 39, 40, 41
  const telefonoIndices = [38, 39, 40, 41];
  const telefonoColExiste = telefonoIndices.some((i) => header.length > i);
  const telefonoValorExiste = telefonoIndices.some((i) => {
    const v = first[i];
    return v != null && String(v).trim() !== "";
  });

  if (!telefonoColExiste || !telefonoValorExiste) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: "parámetros inválidos: bucket y path (string no vacío)",
        }),
        {
          status: 400,
      ...
      return new Response(
        JSON.stringify({
          error: `error descargando archivo: ${
            downloadError?.message || "blob vacío"
          }`,
        }),
        {
          status: 500,
    ...
    if (totalRowsExpected > MAX_EXCEL_ROWS) {
      return new Response(
        JSON.stringify({
          error: `Excel demasiado grande: ${totalRowsExpected} filas (máximo ${MAX_EXCEL_ROWS})`,
        }),
        {
          status: 413,
          headers: { "content-type": "application/json", ...corsHeaders },
    ...
    const validacion = validarColumnasRequeridas(rows);
    if (!validacion.valido) {
      return new Response(
        JSON.stringify({
          error: `Columnas requeridas faltantes o vacías: ${validacion.faltantes.join(
            ", "
          )}`,
        }),
        {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      personas.push({
        ...
        direccionCompleta: `${String(row[30] || "").trim()} ${String(
          row[31] || ""
        ).trim()}`.trim(),
    ...
    for (const p of personas) {
      ...
      if (keyCliente) {
        if (!byCliente.has(keyCliente)) {
          byCliente.set(keyCliente, {
            ...p,
            nrosCliente: [p.nroCliente],
            nrosWO: [p.nroWO],
            cantidadDecos: 1,
          });
        } else {
        ...
        if (!byTelefono.has(keyTel)) {
          byTelefono.set(keyTel, {
            ...p,
            nrosCliente: p.nroCliente ? [p.nroCliente] : [],
            nrosWO: [p.nroWO],
            cantidadDecos: 1,
          });
        } else {
    ...
    for (const v of byTelefono.values()) {
      const exists = deduplicadas.some(
        (d) => d.telefonoNormalizado === v.telefonoNormalizado
      );
    ...
    let kapsoPhoneNumberId: string | null = null;
    {
      const { data: campana } = await supabase
        ...
        .eq("id", campanaId)
        .single();
      if (campana && typeof campana.distancia_max === "number")
        distanciaMax = campana.distancia_max;
      if (campana && typeof campana.kapso_phone_number_id === "string")
        kapsoPhoneNumberId = campana.kapso_phone_number_id;
    }
    ...
    for (let i = 0; i < personasParaInsertar.length; i += chunkSize) {
      ...
      if (insertError) {
        return new Response(
          JSON.stringify({
            error: `error insertando personas: ${insertError.message}`,
          }),
          {
            status: 500,
            headers: { "content-type": "application/json", ...corsHeaders },
    ...
    const MAX_EXPORT_ROWS = parseInt(
      Deno.env.get("MAX_EXPORT_ROWS") || "15000"
    );
    if (
      fueraRangoListado.length > 0 &&
      fueraRangoListado.length <= MAX_EXPORT_ROWS
    ) {
    ...
    // Actualizar campaña
    {
      const { error: updateError } = await supabase
        .from("campanas")
        .update({
          total_personas: deduplicadas.length,
          personas_dentro_rango: dentro,
        })
        .eq("id", campanaId);
      if (updateError) {
        return new Response(
          JSON.stringify({
            error: `error actualizando campaña: ${updateError.message}`,
          }),
          {
            status: 500,
            headers: { "content-type": "application/json", ...corsHeaders },
