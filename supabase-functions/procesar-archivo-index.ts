...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (puntosError) {
      throw new Error(
        `error consultando puntos pickit: ${puntosError.message}`
      );
    }
    ...
    for (const persona of personasDeduplicated) {
      const { punto, distancia } = encontrarPuntoMasCercano(
        persona,
        puntosPickit
      );
    ...
    return new Response(
      JSON.stringify({
        ...
        personas_dentro_rango: personasDentroRango,
        porcentaje:
          personasDeduplicated.length > 0
            ? Math.round(
                (personasDentroRango / personasDeduplicated.length) * 1000
              ) / 10
            : 0,
      }),
      {
