    ...
    if (lat != null && lon != null) {
      ...
      if (!isNaN(latNum) && !isNaN(lonNum)) {
        ...
        const telefonos = [
          ...
          row[39],
          row[40]
        ].filter((t)=>t && String(t).trim()).map((t)=>String(t).trim());
...
function deduplicarPersonas(personas) {
...
function encontrarPuntoMasCercano(persona, puntos) {
  let distanciaMinima = Infinity;
  let puntoMasCercano = null;
  
  for (const punto of puntos){
    const distancia = calcularDistanciaHaversine(persona.lat, persona.lon, punto.lat, punto.lon);
  ...
  try {
    ...
    if (!campana_id || !bucket || !path) {
      return new Response(JSON.stringify({
        error: 'faltan parámetros: campana_id, bucket, path'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: puntosPickit, error: puntosError } = await supabase.from('puntos_pickit').select('id, nombre, direccion, lat, lon');
    if (puntosError) {
      throw new Error(`error consultando puntos pickit: ${puntosError.message}`);
    }
    
    const { data: campana, error: campanaError } = await supabase.from('campanas').select('distancia_max').eq('id', campana_id).single();
    if (campanaError) {
    ...
    for (const persona of personasDeduplicated){
      ...
      personasParaInsertar.push({
        ...
        nro_cliente: persona.nroCliente, // primer elemento (compatibilidad)
        nro_wo: persona.nroWO, // primer elemento (compatibilidad)
        nros_cliente: persona.nrosCliente, // array completo
        nros_wo: persona.nrosWO, // array completo
        cantidad_decos: persona.cantidadDecos, // contador
    ...
    const { error: insertError } = await supabase.from('personas_contactar').insert(personasParaInsertar);
    if (insertError) {
      throw new Error(`error insertando personas: ${insertError.message}`);
    }
    
    const { error: updateError } = await supabase.from('campanas').update({
    ...
    return new Response(JSON.stringify({
      ...
      personas_dentro_rango: personasDentroRango,
      porcentaje: personasDeduplicated.length > 0 ? Math.round(personasDentroRango / personasDeduplicated.length * 1000) / 10 : 0
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
