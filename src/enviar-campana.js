...
function estaDentroDeHorario(campana, timeInfo) {
  ...
  if (diaSemana === 6) {
    ...
    return estaEntreHoras(hora, minuto, inicio.hora, inicio.minuto, fin.hora, fin.minuto)
  }
  ...
  if (diaSemana >= 1 && diaSemana <= 5) {
    ...
    if (estaEntreHoras(hora, minuto, v1Inicio.hora, v1Inicio.minuto, v1Fin.hora, v1Fin.minuto)) {
      return true
    }
    ...
    if (estaEntreHoras(hora, minuto, v2Inicio.hora, v2Inicio.minuto, v2Fin.hora, v2Fin.minuto)) {
      return true
    }
  }
...
function estaEntreHoras(hora, minuto, inicioHora, inicioMinuto, finHora, finMinuto) {
...
async function consultarSupabase(env, query) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`error consultando supabase: ${response.status} - ${errorText}`)
  }
...
async function actualizarSupabase(env, table, data, filter) {
  ...
  if (!response.ok) {
    const errorText = await response.text()
    throw new Error(`error actualizando supabase: ${response.status} - ${errorText}`)
  }
 ...
 */
async function enviarKapsoWorkflow(persona, campana, env, esRecordatorio = false) {
  if (DRY_RUN) {
    console.log('[DRY_RUN] Simulando envío a Kapso:', {
      persona_id: persona.id,
      telefono: persona.telefono_principal,
      workflow: esRecordatorio ? campana.kapso_workflow_id_recordatorio : campana.kapso_workflow_id
    })
  ...
  if (!workflowId) {
    throw new Error(`Workflow ID no configurado para ${esRecordatorio ? 'recordatorio' : 'contacto principal'}`)
  }
  ...
  const nrosCliente = persona.nros_cliente || (persona.nro_cliente ? [persona.nro_cliente] : [])
  ...
  const body = {
    workflow_execution: {
      ...
      phone_number_id: campana.kapso_phone_number_id || env.KAPSO_PHONE_NUMBER_ID,
      ...
      context: {
        source: esRecordatorio ? 'sistema_pickit_recordatorio' : 'sistema_pickit',
  ...
  if (!response.ok) {
    ...
    throw new Error(`error kapso: ${response.status} - ${JSON.stringify(errorData)}`)
  }
...
async function procesarCampana(env, campanaId, esManual = true) {
  ...
  try {
    ...
    log.pasos.push({ paso: 1, accion: 'campaña obtenida', nombre: campana.nombre })
    ...
    if (campana.fecha_fin_contactacion) {
      ...
      if (hoy > fechaFin) {
        log.pasos.push({ paso: 2, resultado: 'fecha_fin_contactacion ya pasó, no se contactará' })
    ...
    if (esManual && esMismoDiaCreacion(campana.created_at, timeInfo)) {
      log.pasos.push({ paso: 2, resultado: 'mismo día de creación, no se contacta' })
    ...
    log.pasos.push({ paso: 3, resultado: 'ok', personas_encontradas: personas.length })
      ...
      const promesas = batch.map(async (persona) => {
        try {
          ...
          await actualizarSupabase(
            env,
            'personas_contactar',
            { estado_contacto: 'error_envio', intentos_envio: (persona.intentos_envio || 0) + 1 },
            new URLSearchParams({ id: `eq.${persona.id}` })
          )
...
async function generarCorteDiario(env, campanaId) {
  ...
  try {
    log.pasos.push({ paso: 1, accion: 'llamando edge function generar-corte-diario' })

    // Llamar a Supabase Edge Function
    const supabaseUrl = env.SUPABASE_URL
    const supabaseKey = env.SUPABASE_KEY

    const response = await fetch(`${supabaseUrl}/functions/v1/generar-corte-diario`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      const errorText = await response.text()
      throw new Error(`Error generando corte diario: ${response.status} - ${errorText}`)
    }
...
async function procesarRecordatorios(env) {
  ...
  try {
    ...
    log.pasos.push({ paso: 1, accion: 'buscando confirmados con fecha_compromiso = hoy' })
    
    // Consultar todas las campañas activas
    const campanasActivas = await consultarSupabase(env, {
      table: 'campanas',
      select: 'id, nombre, kapso_workflow_id_recordatorio, kapso_phone_number_id',
    ...
    for (const campana of campanasActivas) {
      ...
      const personas = await consultarSupabase(env, {
        table: 'personas_contactar',
        select: 'id, campana_id, apellido_nombre, telefono_principal, nro_cliente, nro_wo, nros_wo, cantidad_decos, fecha_compromiso, punto_pickit_id',
        params: new URLSearchParams({
      ...
      const puntoIds = [...new Set(personas.map(p => p.punto_pickit_id).filter(Boolean))]
      let puntosPickit = {}
      if (puntoIds.length > 0) {
        ...
        puntosPickit = puntos.reduce((acc, p) => { acc[p.id] = p; return acc }, {})
      }
      ...
      for (const persona of personasConPunto) {
        try {
          ...
          resultados.push({ success: true, persona_id: persona.id, campana_id: campana.id })
        } catch (error) {
          errores.push({ success: false, persona_id: persona.id, campana_id: campana.id, error: error.message })
        }
      }
    }
...
export default {
  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      ...
      // 2. Procesar contacto inicial (12:00 UTC = inicio ventana 1 AR)
