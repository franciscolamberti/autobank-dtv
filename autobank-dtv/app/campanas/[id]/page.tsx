...
interface PersonasSections {
  comprometidos: PersonaConPickit[]; // PRD bucket 1: confirmado con fecha_compromiso
  inProgress: PersonaConPickit[]; // PRD bucket 2: encolado, enviado_whatsapp, respondio
  fueraDeRango: PersonaConPickit[]; // PRD bucket 3: fuera_de_rango = true
  sinWhatsapp: PersonaConPickit[]; // PRD bucket 4: tiene_whatsapp = false
  atencionEspecial: PersonaConPickit[]; // PRD bucket 5: rechazado OR solicita_retiro_domicilio
}
...
export default function CampaignDetailPage() {
  ...
  const loadCortes = async () => {
    try {
      ...
      toast({
        title: "No se pudieron cargar los cortes diarios",
        variant: "destructive",
      });
    }
  };
    ...
    try {
      ...
      personasData?.forEach((persona: any) => {
        ...
        const personaNormalizada = {
          ...persona,
          punto_pickit: Array.isArray(persona.punto_pickit)
            ? persona.punto_pickit[0]
        ...
        // Bucket 2: In Progress - encolado, enviado_whatsapp, respondio
          ...
          ["encolado", "enviado_whatsapp", "respondio"].includes(
            personaNormalizada.estado_contacto
          )
        ) {
      ...
      toast({
        title: "Error al cargar los datos de la campaña",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleSendMessages = async () => {
    ...
    try {
      const WORKER_URL =
        ...
        "https://enviar-campana.your-worker.workers.dev";
      ...
      toast({
        title: `Mensajes iniciados. ${result.resumen?.exitosas || 0} enviados`,
      });
      ...
      toast({
        title: "Error al enviar mensajes. Por favor intente nuevamente.",
        variant: "destructive",
      });
    } finally {
      setSending(false);
  ...
  const handleToggleDevolucion = async (
    personaId: string,
    ...
    try {
      ...
      toast({
        title: "Error al actualizar el estado de devolución",
        variant: "destructive",
      });
    }
  };

  const handleExportBucket = async (
    bucketName: string,
    personas: Tables<"personas_contactar">[]
  ) => {
    if (personas.length === 0) {
      toast({
        title: `No hay personas en el bucket ${bucketName}`,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const excelData = personas.map((persona: any) => ({
        ...
        "Solicita Retiro Domicilio": persona.solicita_retiro_domicilio
          ? "Sí"
          : "No",
        ...
        ...(bucketName === "SinWhatsapp"
          ? {
              "Motivo Error":
                persona.error_envio_kapso || "número sin whatsapp",
            }
          : {}),
      }));
      ...
      const fileName = `${campaign?.nombre || "campaign"}-${bucketName}-${
      ...
      toast({
        title: `Error al generar el export de ${bucketName}`,
        variant: "destructive",
      });
    }
  };
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const getSeccion = (persona: any): string => {
        if (
          persona.dentro_rango &&
          persona.estado_contacto === "enviado_whatsapp"
        ) {
          return "Contactados";
          ...
          ["pendiente", "encolado", "respondio", "no_responde"].includes(
            persona.estado_contacto
          )
        ) {
          return "En Progreso";
        }
      ...
      const excelData =
        personasData?.map((persona: any) => ({
          ...
          "Fecha Respuesta": persona.fecha_respuesta
            ...
            : "",
          "Respuesta Texto": persona.respuesta_texto || "",
          "Decodificador Devuelto": persona.decodificador_devuelto
            ? "Sí"
            : "No",
      ...
      // Generate filename
      const fileName = `campana-${
        campaign?.nombre?.replace(/[^a-z0-9]/gi, "_").toLowerCase() || "export"
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-4">
            La campaña que buscas no existe o fue eliminada.
          </p>
          <Link href="/">
            ...
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Personas confirmadas con fecha de compromiso. Aparecen en
                    archivo diario Pickit en el corte del día del compromiso
                  </CardDescription>
                </CardHeader>
                <CardContent>
                          ...
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm font-semibold text-green-700 mt-1">
                                Comprometido:{" "}
                                {persona.fecha_compromiso
                                  ? formatIsoDateToDmy(persona.fecha_compromiso)
                                  : "Sin fecha"}
                              </p>
                              {persona.punto_pickit && (
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleExportBucket("InProgress", personas.inProgress)
                      }
                  ...
                  <CardDescription>
                    Conversación activa: encolado, enviado WhatsApp, o respondió
                  </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        >
                          ...
                          <div className="text-right">
                            <Badge variant="outline" className="capitalize">
                              {persona.estado_contacto === "encolado" &&
                                "Encolado"}
                              {persona.estado_contacto === "enviado_whatsapp" &&
                                "Enviado"}
                              {persona.estado_contacto === "respondio" &&
                                "Respondió"}
                              {persona.estado_contacto === "pendiente" &&
                                "Pendiente"}
                            </Badge>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Distancia mayor a {campaign?.distancia_max || 2000}m. Export
                    generado al crear campaña
                  </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        >
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <div className="flex-1">
                              ...
                              {persona.punto_pickit && (
                                <p className="text-xs text-muted-foreground mt-1">
                                  Más cercano: {persona.punto_pickit.nombre} (
                                  {Math.round(persona.distancia_metros || 0)}m)
                                </p>
                              )}
                            </div>
                          </div>
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleExportBucket("SinWhatsapp", personas.sinWhatsapp)
                      }
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="secondary">
                        {personas.atencionEspecial.length}
                      </Badge>
                    </div>
                    <Button
                      variant="outline"
                  ...
                  <CardDescription>
                    Rechazados o solicita retiro domicilio. Motivo negativo
                    generado por agente Kapso
                  </CardDescription>
                </CardHeader>
                <CardContent>
